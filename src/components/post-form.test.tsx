...
it.each([
  ["handle", "Handle (optional)", "bry"],
  ["post", "Post", "One more thought"],
])("clears failed feedback when the %s changes", async (_, label, edit) => {
  mocks.createPost.mockResolvedValueOnce({ error: "Please revise this post." });
  const user = userEvent.setup();
  render(<PostForm />);
  const post = screen.getByLabelText("Post");
  await user.type(post, "Original draft");
  await user.click(screen.getByRole("button", { name: "Post" }));
  expect(await screen.findByRole("alert")).toHaveTextContent(
  ...
  await user.type(screen.getByLabelText(label), edit);

  expect(screen.queryByRole("alert")).not.toBeInTheDocument();
  expect(post).toHaveAttribute("aria-invalid", "false");
  expect(post).toHaveValue(
    label === "Post" ? `Original draft${edit}` : "Original draft",
  );
});

it.each([
  ["handle", "Handle (optional)", "next-author"],
  ["post", "Post", "Next draft"],
])("clears successful feedback when the %s changes", async (_, label, edit) => {
  mocks.createPost.mockResolvedValueOnce({ ok: true });
  const user = userEvent.setup();
  render(<PostForm />);
  await user.type(screen.getByLabelText("Post"), "Saved draft");
  await user.click(screen.getByRole("button", { name: "Post" }));
  expect(await screen.findByRole("status")).toHaveTextContent("Quirk posted.");
...
it("shows new feedback when an edited draft is resubmitted", async () => {
  mocks.createPost
    .mockResolvedValueOnce({ error: "First response." })
    .mockResolvedValueOnce({ error: "Second response." });
  const user = userEvent.setup();
  render(<PostForm />);
  const post = screen.getByLabelText("Post");
  await user.type(post, "Draft");
  await user.click(screen.getByRole("button", { name: "Post" }));
  expect(await screen.findByRole("alert")).toHaveTextContent("First response.");
  await user.type(post, " revised");
  expect(screen.queryByRole("alert")).not.toBeInTheDocument();
  ...
  expect(post).toHaveValue("Draft revised");
});
